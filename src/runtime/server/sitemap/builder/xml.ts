...
import { xmlEscape } from '../../../utils-pure'
...
export function* renderSitemapXmlChunks(
  ...
  if (credits) {
    yield `${NL}<!-- XML Sitemap generated by @nuxtjs/sitemap v${version} at ${new Date().toISOString()} -->`
  }
}
...
export function urlsToXml(
  ...
  let xml = ''
  for (const chunk of renderSitemapXmlChunks(urls, resolvers, config, errorInfo))
    xml += chunk
  return xml
}

export function urlsToXmlStream(
  urls: ResolvedSitemapUrl[],
  resolvers: NitroUrlResolvers,
  config: Pick<ModuleRuntimeConfig, 'version' | 'xsl' | 'credits' | 'minify'>,
  errorInfo?: { messages: string[], urls: string[] },
