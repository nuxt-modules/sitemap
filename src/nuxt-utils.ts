...
export function exposeVirtualRecord(fileName: string, config: Record<string, any>) {
  const nuxt = useNuxt()
  const jsExports = Object.entries(config).map(([k, v]) => `export const ${k} = ${JSON.stringify(v)}`).join('\n')
  const alias = `#virtual/${fileName}`
  nuxt.options.alias[alias] = addTemplate({
    filename: `modules/virtual/${fileName}.mjs`,
    getContents: () => jsExports,
  }).dst
  // expose for nitro
  nuxt.hooks.hook('nitro:config', (nitroConfig) => {
  ...
  // need to create a ts def for the file
  const typeDefName = `virtual/${fileName}.d.ts`
  const tsExports = Object.keys(config).map((k) => {
    // expose actual values in dev
    if (nuxt.options.dev)
      return `  export const ${k}: ${JSON.stringify(config[k])} | ModuleOptions['${k}']`
...
import type { ModuleOptions } from '${fileName}'
declare module '${alias}' {
${tsExports}
}
`
