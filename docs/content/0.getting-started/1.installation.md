...
::note
Already using `@nuxtjs/seo`? Do not install this again unless you want to manage the Sitemap module separately. `@nuxtjs/seo` includes `@nuxtjs/sitemap`.
::
...
All pages present in your app are discovered from your [Application Sources](/docs/sitemap/getting-started/data-sources). For CMS, database, or API-backed routes, see [Dynamic URLs](/docs/sitemap/guides/dynamic-urls).
...
Every site is different and will require its own further unique configuration, to give you a head start:

- [Dynamic URL Endpoint](/docs/sitemap/guides/dynamic-urls) - If you have dynamic URLs you need to add to the sitemap, you can use a runtime API endpoint. For example, if you're
