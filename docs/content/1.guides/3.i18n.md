...
**Custom sitemaps with I18n.** You can add custom sitemaps alongside the automatic i18n multi-sitemap. When any sitemap uses `includeAppSources: true`, the module still generates per-locale sitemaps and merges the `exclude`/`include` filters:
...
**Custom path translations.** If you have custom path translations defined in your i18n configuration using `pages`, the `_i18nTransform` option will automatically use them:
...
For more customization options, see the [Customizing UI guide](/docs/sitemap/advanced/customising-ui).
