...
**Supported content types.** The sitemap integration works with all content file types supported by Nuxt Content:
- Markdown (`.md`)
- YAML (`.yml` / `.yaml`)
- JSON (`.json`)
- CSV (`.csv`)
...
[comark-content](https://github.com/harlan-zw/harlan-nuxt/tree/main/packages/comark-content) is a Markdown-only content module. Nuxt Sitemap supports it first-party, with its own data source named `@harlan-zw/comark-content:urls`.

No schema opt in is needed. Every page collection is in the sitemap by default.
...
To keep a collection out of the sitemap, set `sitemap: false`{lang="ts"} on the collection.
...
The `sitemap` frontmatter key works the same as it does with Nuxt Content v3. See [Usage](#usage).

To exclude every comark URL, add the source to `excludeAppSources`.

```ts [nuxt.config.ts]
export default defineNuxtConfig({
  sitemap: {
    excludeAppSources: ['@harlan-zw/comark-content:urls'],
  },
})
```

::caution
comark-content 0.1.4 or later is required. Earlier versions add their own URLs, so pairing one with this module lists every page twice.
::
...
**Advanced: Nuxt Content app source.** If you'd like to set up a more automated Nuxt Content integration and you're not using Document Driven mode, you can add content to the sitemap as you would with [Dynamic URLs](/docs/sitemap/guides/dynamic-urls).
...
**Troubleshooting exclusions.** If `sitemap: false` or `robots: false` aren't working, check the following:
