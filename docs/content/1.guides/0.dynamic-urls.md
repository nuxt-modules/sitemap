...
Use `defineSitemapEventHandler()`{lang="ts"} for every sitemap source endpoint. It is a typed wrapper around Nitro's `defineEventHandler()`{lang="ts"}. It types the return value as `SitemapUrlInput[]`{lang="ts"} so your editor checks each URL object.

By default it adds no caching, because the module already caches the resolved sitemap in production. [`cacheMaxAgeSeconds`](/docs/sitemap/advanced/performance#cache-time) (10 minutes by default) covers source fetching, normalization and sorting. All chunks of the same sitemap share one cache entry, so chunking does not multiply requests to your endpoint.

Your endpoint is still called once per cache window, for each named sitemap that lists it. The module cache is disabled in development and during prerendering, so every request reaches your endpoint there.

Pass cache options as the second argument when the upstream data is slow, rate limited, or metered, or when several named sitemaps read the same endpoint. The options match Nitro's [`defineCachedEventHandler`](https://nitro.build/guide/cache):
...
You never need `defineCachedEventHandler()`{lang="ts"} for a sitemap source. It works, but you lose the URL types.
