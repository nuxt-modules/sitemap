...
- Sitemaps that list the same source URL share one fetch, so 20 sitemaps reading `/api/__sitemap__/urls` call it once per cache window, not 20 times.

The module cache is disabled in development and during prerendering, so every request reaches your endpoint there.

Reach for Nitro's `defineCachedEventHandler()`{lang="ts"} only when your endpoint is expensive enough that one call per cache window still hurts, for example a slow, rate limited, or metered upstream API. It has no sitemap-aware types, so annotate the return value:
