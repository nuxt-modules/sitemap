...
Sitemaps defined in `nuxt.config` are fixed at build time. When your data decides how many sitemaps you need — a catalog that grows while the server runs — register them at runtime with the [`sitemap:sitemaps-resolved`](/docs/sitemap/nitro-api/nitro-hooks) hook.
