...
**Customizing sitemap URLs.** By default, all multi-sitemaps are served under the `/__sitemap__/` prefix. You can customize this behavior to create cleaner URLs:
...
**Using the `_sitemap` key.** When using global sources and need to direct specific URLs to particular sitemaps, use the `_sitemap` key:
...
Sitemaps in `nuxt.config` are fixed at build time. If your data decides how many you need, register them at runtime instead. A catalog that grows while the server runs is the typical case.

Register sitemaps with the [`sitemap:sitemaps-resolved`](/docs/sitemap/nitro-api/nitro-hooks) hook:
...
See [Sitemap Chunking: When built-in chunking isn't enough](/docs/sitemap/advanced/chunking-sources) for chunking strategies, removal, and caching behavior.
