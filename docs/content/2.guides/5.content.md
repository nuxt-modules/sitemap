...
If `sitemap: false` or `robots: false` isn't working, verify:

1. **Nuxt Content v3**: You've wrapped your collection with `asSitemapCollection()` (see [Setup Nuxt Content v3](#setup-nuxt-content-v3) above)
2. **Nuxt Content v2**: You have `documentDriven: true` or `strictNuxtContentPaths: true` enabled (see [Setup Nuxt Content v2](#setup-nuxt-content-v2) above)
3. **Module load order**: `@nuxtjs/sitemap` must be listed before `@nuxt/content` in your modules array

If pages still appear after these checks, clear `.nuxt` and restart dev.
