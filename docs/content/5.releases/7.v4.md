...
relatedPages:
  - path: /docs/sitemap/releases/v5
    title: Nuxt Sitemap v5.0.0
  - path: /docs/sitemap/releases/v3
    title: v3.0.0
...
Every URL within your sitemap will belong to a source. A source will either be a User source or an Application source.
...
Now utilizes native route rules. By default will set up SWR rules for 10 minutes.
...
- The config `trailingSlash` and `siteUrl` have been removed. You should use site config, see [Setting Site Config](/docs/site-config/guides/how-it-works).
