...
For the lowest time-to-first-byte and peak serialization memory, enable streaming and compression together:
...
Streaming applies to dynamic responses. Prerendering still produces a complete static file. URL collection, filtering, deduplication, sorting, and the `sitemap:resolved` hook also finish before the first XML chunk is sent so errors can still return a proper HTTP response.
