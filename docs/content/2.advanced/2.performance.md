...
2. **Cache the source endpoint when the upstream is expensive.** [Sitemap Caching](#sitemap-caching) already keeps your `/api/*` sources off most requests, and sitemaps that share a source URL share one fetch of it. Add `defineCachedEventHandler` on top of that only when the upstream API is slow, rate limited, or metered. See [Which handler should I use?](/docs/sitemap/guides/dynamic-urls#which-handler-should-i-use).
