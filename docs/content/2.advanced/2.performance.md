...
2. **Cache the source endpoint when the upstream is expensive.** [Sitemap Caching](#sitemap-caching) already keeps your `/api/*` sources off most requests. Add cache options to `defineSitemapEventHandler()` on top of it when the upstream API is slow, rate limited, or metered, or when several named sitemaps read the same endpoint. See [Which handler should I use?](/docs/sitemap/guides/dynamic-urls#which-handler-should-i-use).
