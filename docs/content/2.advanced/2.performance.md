...
Source endpoints created with `defineSitemapEventHandler` are not cached themselves. The helper is a typed event handler. The module caches the resolved URL set and the rendered XML, so your endpoint only runs when that cache expires.
