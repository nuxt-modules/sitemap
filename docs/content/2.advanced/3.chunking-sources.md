...
Built-in chunking splits a sorted URL list by position. Delete one URL and every chunk after it shifts: each file gets new content, but their `lastmod` dates can't say so. If you need honest `lastmod` dates, or your data grows past a fixed set of sitemaps while the server runs, chunk at the source instead and register the chunks yourself with the [`sitemap:sitemaps-resolved`](/docs/sitemap/nitro-api/nitro-hooks) hook.

The idea: decide chunk boundaries in your database, give each chunk a source endpoint that returns just its own URLs, and register one sitemap per chunk.
...
The source endpoint serves one chunk per request, so a chunk only pays for its own URLs. Set the index `lastmod` per chunk from your data with the [`sitemap:index-resolved`](/docs/sitemap/nitro-api/nitro-hooks) hook.

### Chunk by stable ranges

Chunk by stable ID ranges, not by position. If chunk 2 holds games 10001 to 15000, it should always hold those games. Delete game 10002 and chunk 2 keeps its boundaries, so its `lastmod` only changes when its own content changes. Positional chunks shift on every delete, which makes their `lastmod` meaningless.
...
Growth needs no special handling — the next run registers the new chunk. Shrinkage is the same in reverse: stop registering a chunk and it leaves the index, its route returns a 404. `delete` is only needed for static sitemaps you declared in `nuxt.config` and no longer want.
...
Keep one timing detail in mind after a removal: the sitemap list and the sitemap index refresh on their own schedules. For up to one cache window, the index may still list a sitemap whose route already returns a 404. Crawlers retry; if the window feels too long for your traffic, lower `cacheMaxAgeSeconds`.
