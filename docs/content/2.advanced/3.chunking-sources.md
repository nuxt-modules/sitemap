...
Switch to `defineCachedEventHandler()`{lang="ts"} when the query itself is expensive. Annotate the return value, because this handler has no sitemap-aware types:
